...
import hashlib
import re  # noqa: F401  (kept for backward-compat module API)
    ...
    for row in annotation.iter_rows():
        ...
        if (
            ...
            if ".exon." in d["ID"]:
                ...
                transcript_id = transcript_id.removeprefix("exon-").removeprefix("exon")
            else:
                continue
        else:
            continue
        ...
        if (gene_biotype := gene_info[gene_id].get("gene_biotype", "")) != "" and d.get(
        ...
        if feature_type == "exon":
            tx.add_exon(exon_id, Span(row.start - 1, row.end))
        elif feature_type == "start_codon":
            tx.start_codon = Span(row.start - 1, row.end)
        elif feature_type == "stop_codon":
            tx.stop_codon = Span(row.start - 1, row.end)
        else:
            continue
