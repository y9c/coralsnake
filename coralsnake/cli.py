...
@cli.command(
    help="[DEBUG] Sequential mapping without multiprocessing (for debugging). Same as 'map' but processes reads sequentially.",
    no_args_is_help=True,
    context_settings=dict(help_option_names=["-h", "--help"]),
)
@click.option(
...
@click.option("-2", "--r2-file", help="r2 file", required=False)
@click.option("-o", "--output-file", help="output bam file", required=False)
@click.option("--max-mismatches", "-m", type=int, default=10)
@click.option("--threads", "-t", type=int, default=1, help="Ignored in sequential mode")
@click.option("--min-alignment-length", type=int, default=20)
@click.option("--min-mapping-ratio", type=float, default=0.5)
...
@click.option("--fwd-lib", "--forward-library", "library_type", flag_value="forward", default=True)
@click.option("--rev-lib", "--reverse-library", "library_type", flag_value="reverse")
@click.option("--fwd-ref", "--forward-reference", "reference_strand", flag_value="forward")
@click.option("--rev-ref", "--reverse-reference", "reference_strand", flag_value="reverse")
@click.option("--dbl-ref", "--double-reference", "reference_strand", flag_value="double", default=True)
def map1(
    ref_files,
    r1_file,
    r2_file,
    ...
    from .mapping1 import map_file
    import os

    # Convert tuple to list
    ref_files = list(ref_files) if ref_files else []

    # Same validation as map command
    if index_only:
        if not index_dir:
        ...
        if not ref_files:
            ...
            if os.path.exists(os.path.join(index_dir, "ref.orig.bwt")) and os.path.exists(os.path.join(index_dir, "ref.mk.bwt")):
                ...
                idx_files_exist = True
            if not idx_files_exist:
                click.echo("❌ Error: --ref-file is required because BWA indices not found in --index-dir", err=True)
                raise click.Abort()
    ...
    try:
        map_file(
            ...
            forward_library,
            max_mismatches,
            threads,  # Ignored in sequential mode
            min_alignment_length,
            min_mapping_ratio,
            index_dir,
            index_only,
