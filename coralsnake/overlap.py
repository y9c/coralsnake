...
def calculate_bin_statistics(
    ...
    # Create column names
    col_names = {"count": "count", "sum": "sum", "mean": "mean"}
    if suffix:
    ...
    ).with_columns(pl.Series("index", (bins[1:] + bins[:-1]) / 2))
...
def annotate_with_features(
    ...
    df_joined = overlaps_df.join(input_indexed, on="input_idx").join(
    ...
    df = df.with_columns(
        ...
        .then(
            ((pl.col("Start") + pl.col("End")) // 2).clip(
                pl.col("Start_b"), pl.col("End_b")
            )
            - pl.col("Start_b")
        )
        .otherwise(
    ...
    if type_ratios is not None:
        ...
        s = (
            matching_features.group_by(["Transcript", "Type"])
